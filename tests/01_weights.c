...
static srukf_value expected_lambda(srukf_value alpha, srukf_value kappa,
                                 srukf_index N) {
  return alpha * alpha * ((srukf_value)N + kappa) - (srukf_value)N;
}

/* ---------- helper: compute expected weight vectors --------------- */
static void compute_expected_weights(srukf_value lambda, srukf_value alpha,
...
static void test_basic(void) {
  ...
  srukf_value lambda = expected_lambda(1e-3, 0.0, n);

  srukf_value *wm_exp = (srukf_value *)calloc(2 * n + 1, sizeof(srukf_value));
  srukf_value *wc_exp = (srukf_value *)calloc(2 * n + 1, sizeof(srukf_value));
...
static void test_large_dim(void) {
  ...
  srukf_index n = ukf->x->n_rows;
  srukf_value lambda = expected_lambda(1.0, 1.0, n);
...
static void test_negative_lambda(void) {
  ...
  srukf_index n = ukf->x->n_rows;
  srukf_value lambda = expected_lambda(0.1, -0.5, n);

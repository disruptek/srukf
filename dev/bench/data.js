window.BENCHMARK_DATA = {
  "lastUpdate": 1770504443632,
  "repoUrl": "https://github.com/disruptek/srukf",
  "entries": {
    "Benchmark": [
      {
        "commit": {
          "author": {
            "email": "github@andy.disruptek.com",
            "name": "Smooth Operator",
            "username": "disruptek"
          },
          "committer": {
            "email": "github@andy.disruptek.com",
            "name": "Smooth Operator",
            "username": "disruptek"
          },
          "distinct": true,
          "id": "1937027a09488bf42ebee1e6028c428f384c9b97",
          "message": "Fix benchmark workflow: improve parsing robustness and add debug output",
          "timestamp": "2026-02-07T17:18:44-05:00",
          "tree_id": "0ca5f302a3516678f5ee51d76fbd6a614d61a809",
          "url": "https://github.com/disruptek/srukf/commit/1937027a09488bf42ebee1e6028c428f384c9b97"
        },
        "date": 1770503390306,
        "tool": "customSmallerIsBetter",
        "benches": [
          {
            "name": "3x2/predict",
            "value": 1.432,
            "unit": "us"
          },
          {
            "name": "3x2/predict_to",
            "value": 1.414,
            "unit": "us"
          },
          {
            "name": "3x2/correct",
            "value": 2.25,
            "unit": "us"
          },
          {
            "name": "3x2/correct_to",
            "value": 2.265,
            "unit": "us"
          },
          {
            "name": "6x3/predict",
            "value": 3.287,
            "unit": "us"
          },
          {
            "name": "6x3/predict_to",
            "value": 1.876,
            "unit": "us"
          },
          {
            "name": "6x3/correct",
            "value": 3.149,
            "unit": "us"
          },
          {
            "name": "6x3/correct_to",
            "value": 3.127,
            "unit": "us"
          },
          {
            "name": "10x5/predict",
            "value": 3.662,
            "unit": "us"
          },
          {
            "name": "10x5/predict_to",
            "value": 3.672,
            "unit": "us"
          },
          {
            "name": "10x5/correct",
            "value": 7.367,
            "unit": "us"
          },
          {
            "name": "10x5/correct_to",
            "value": 7.3,
            "unit": "us"
          },
          {
            "name": "15x8/predict",
            "value": 7.228,
            "unit": "us"
          },
          {
            "name": "15x8/predict_to",
            "value": 7.23,
            "unit": "us"
          },
          {
            "name": "15x8/correct",
            "value": 20.057,
            "unit": "us"
          },
          {
            "name": "15x8/correct_to",
            "value": 19.613,
            "unit": "us"
          },
          {
            "name": "20x10/predict",
            "value": 11.698,
            "unit": "us"
          },
          {
            "name": "20x10/predict_to",
            "value": 11.884,
            "unit": "us"
          },
          {
            "name": "20x10/correct",
            "value": 36.903,
            "unit": "us"
          },
          {
            "name": "20x10/correct_to",
            "value": 36.857,
            "unit": "us"
          },
          {
            "name": "nonlin/predict",
            "value": 2.653,
            "unit": "us"
          },
          {
            "name": "nonlin/predict_to",
            "value": 3.365,
            "unit": "us"
          },
          {
            "name": "nonlin/correct",
            "value": 3.124,
            "unit": "us"
          },
          {
            "name": "nonlin/correct_to",
            "value": 3.119,
            "unit": "us"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "email": "github@andy.disruptek.com",
            "name": "Smooth Operator",
            "username": "disruptek"
          },
          "committer": {
            "email": "github@andy.disruptek.com",
            "name": "Smooth Operator",
            "username": "disruptek"
          },
          "distinct": true,
          "id": "7901652fef1336760a745692d0740a7645b6f5be",
          "message": "Fix CMake: be flexible with BLAS library naming (cblas, blas, or openblas)",
          "timestamp": "2026-02-07T17:33:32-05:00",
          "tree_id": "1843e166a9d15d4ad694ec0ba69a07551bce9390",
          "url": "https://github.com/disruptek/srukf/commit/7901652fef1336760a745692d0740a7645b6f5be"
        },
        "date": 1770503689398,
        "tool": "customSmallerIsBetter",
        "benches": [
          {
            "name": "3x2/predict",
            "value": 1.402,
            "unit": "us"
          },
          {
            "name": "3x2/predict_to",
            "value": 1.396,
            "unit": "us"
          },
          {
            "name": "3x2/correct",
            "value": 2.211,
            "unit": "us"
          },
          {
            "name": "3x2/correct_to",
            "value": 2.207,
            "unit": "us"
          },
          {
            "name": "6x3/predict",
            "value": 3.24,
            "unit": "us"
          },
          {
            "name": "6x3/predict_to",
            "value": 1.951,
            "unit": "us"
          },
          {
            "name": "6x3/correct",
            "value": 3.147,
            "unit": "us"
          },
          {
            "name": "6x3/correct_to",
            "value": 3.202,
            "unit": "us"
          },
          {
            "name": "10x5/predict",
            "value": 3.704,
            "unit": "us"
          },
          {
            "name": "10x5/predict_to",
            "value": 3.692,
            "unit": "us"
          },
          {
            "name": "10x5/correct",
            "value": 7.305,
            "unit": "us"
          },
          {
            "name": "10x5/correct_to",
            "value": 7.32,
            "unit": "us"
          },
          {
            "name": "15x8/predict",
            "value": 7.359,
            "unit": "us"
          },
          {
            "name": "15x8/predict_to",
            "value": 7.305,
            "unit": "us"
          },
          {
            "name": "15x8/correct",
            "value": 19.374,
            "unit": "us"
          },
          {
            "name": "15x8/correct_to",
            "value": 19.179,
            "unit": "us"
          },
          {
            "name": "20x10/predict",
            "value": 11.716,
            "unit": "us"
          },
          {
            "name": "20x10/predict_to",
            "value": 11.763,
            "unit": "us"
          },
          {
            "name": "20x10/correct",
            "value": 37.058,
            "unit": "us"
          },
          {
            "name": "20x10/correct_to",
            "value": 36.856,
            "unit": "us"
          },
          {
            "name": "nonlin/predict",
            "value": 2.676,
            "unit": "us"
          },
          {
            "name": "nonlin/predict_to",
            "value": 3.381,
            "unit": "us"
          },
          {
            "name": "nonlin/correct",
            "value": 2.997,
            "unit": "us"
          },
          {
            "name": "nonlin/correct_to",
            "value": 3.166,
            "unit": "us"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "email": "github@andy.disruptek.com",
            "name": "Smooth Operator",
            "username": "disruptek"
          },
          "committer": {
            "email": "github@andy.disruptek.com",
            "name": "Smooth Operator",
            "username": "disruptek"
          },
          "distinct": true,
          "id": "ee20febb2863f43d7e46f613ff117a7beb00300f",
          "message": "Fix CI job failures: memory leaks, coverage target, CMake BLAS, and example CSV output\n\nThis commit fixes all 4 failing CI advisory jobs:\n\n1. **AddressSanitizer (test_10_simple)**: Fixed memory leak in 10_simple.c by adding\n   srukf_mat_free() calls for Qsqrt and Rsqrt matrices in cleanup section.\n\n2. **Coverage target**: Made gcov error-handling more robust by suppressing errors\n   and providing helpful message about using lcov for detailed reports.\n\n3. **CMake config template**: Added flexible BLAS library detection (cblas → blas\n   → openblas) in srukfConfig.cmake.in to match CMakeLists.txt behavior, ensuring\n   downstream find_package(srukf) works across systems with different BLAS naming.\n\n4. **Examples job (GPS+IMU)**: Fixed gps_imu.c CSV output to generate expected\n   filenames (gps_imu_truth.csv and gps_imu_estimate.csv) instead of gps_imu_position.csv\n   to match CI test expectations.",
          "timestamp": "2026-02-07T17:40:23-05:00",
          "tree_id": "ddb9139c84a09fb74a45da4a92a397142fa814b8",
          "url": "https://github.com/disruptek/srukf/commit/ee20febb2863f43d7e46f613ff117a7beb00300f"
        },
        "date": 1770504062611,
        "tool": "customSmallerIsBetter",
        "benches": [
          {
            "name": "3x2/predict",
            "value": 1.427,
            "unit": "us"
          },
          {
            "name": "3x2/predict_to",
            "value": 1.399,
            "unit": "us"
          },
          {
            "name": "3x2/correct",
            "value": 2.24,
            "unit": "us"
          },
          {
            "name": "3x2/correct_to",
            "value": 2.297,
            "unit": "us"
          },
          {
            "name": "6x3/predict",
            "value": 3.351,
            "unit": "us"
          },
          {
            "name": "6x3/predict_to",
            "value": 1.885,
            "unit": "us"
          },
          {
            "name": "6x3/correct",
            "value": 3.164,
            "unit": "us"
          },
          {
            "name": "6x3/correct_to",
            "value": 3.143,
            "unit": "us"
          },
          {
            "name": "10x5/predict",
            "value": 3.665,
            "unit": "us"
          },
          {
            "name": "10x5/predict_to",
            "value": 3.645,
            "unit": "us"
          },
          {
            "name": "10x5/correct",
            "value": 7.325,
            "unit": "us"
          },
          {
            "name": "10x5/correct_to",
            "value": 7.339,
            "unit": "us"
          },
          {
            "name": "15x8/predict",
            "value": 7.2,
            "unit": "us"
          },
          {
            "name": "15x8/predict_to",
            "value": 7.155,
            "unit": "us"
          },
          {
            "name": "15x8/correct",
            "value": 19.278,
            "unit": "us"
          },
          {
            "name": "15x8/correct_to",
            "value": 19.588,
            "unit": "us"
          },
          {
            "name": "20x10/predict",
            "value": 11.554,
            "unit": "us"
          },
          {
            "name": "20x10/predict_to",
            "value": 11.605,
            "unit": "us"
          },
          {
            "name": "20x10/correct",
            "value": 38.825,
            "unit": "us"
          },
          {
            "name": "20x10/correct_to",
            "value": 36.78,
            "unit": "us"
          },
          {
            "name": "nonlin/predict",
            "value": 2.667,
            "unit": "us"
          },
          {
            "name": "nonlin/predict_to",
            "value": 3.344,
            "unit": "us"
          },
          {
            "name": "nonlin/correct",
            "value": 3.149,
            "unit": "us"
          },
          {
            "name": "nonlin/correct_to",
            "value": 3.132,
            "unit": "us"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "email": "github@andy.disruptek.com",
            "name": "Smooth Operator",
            "username": "disruptek"
          },
          "committer": {
            "email": "github@andy.disruptek.com",
            "name": "Smooth Operator",
            "username": "disruptek"
          },
          "distinct": true,
          "id": "321b73feaff2e0d9374c2814194df65827c1b576",
          "message": "Fix stability example CSV output and CMake downstream find_package\n\nTwo fixes for remaining CI job failures:\n\n1. **Stability example CSV output**: Fixed logic error where CSV generation code\n   was inside a negated conditional (format != CSV), making it unreachable.\n   Restructured output generation to properly generate CSV when requested.\n   Stability now correctly generates stability_results.csv with all timestep data.\n\n2. **CMake downstream find_package target**: Added srukf::srukf alias target in\n   srukfConfig.cmake.in that points to the primary available target (shared or\n   static). This allows downstream projects to use find_package(srukf) and\n   target_link_libraries(exe srukf::srukf) without caring about implementation details.\n   The config intelligently aliases to srukf_shared if available, otherwise static.",
          "timestamp": "2026-02-07T17:44:47-05:00",
          "tree_id": "7dfbacf8d82313e3e207241ac80386ebaaa1fe02",
          "url": "https://github.com/disruptek/srukf/commit/321b73feaff2e0d9374c2814194df65827c1b576"
        },
        "date": 1770504442968,
        "tool": "customSmallerIsBetter",
        "benches": [
          {
            "name": "3x2/predict",
            "value": 1.629,
            "unit": "us"
          },
          {
            "name": "3x2/predict_to",
            "value": 1.604,
            "unit": "us"
          },
          {
            "name": "3x2/correct",
            "value": 2.307,
            "unit": "us"
          },
          {
            "name": "3x2/correct_to",
            "value": 2.3,
            "unit": "us"
          },
          {
            "name": "6x3/predict",
            "value": 2.541,
            "unit": "us"
          },
          {
            "name": "6x3/predict_to",
            "value": 1.826,
            "unit": "us"
          },
          {
            "name": "6x3/correct",
            "value": 2.709,
            "unit": "us"
          },
          {
            "name": "6x3/correct_to",
            "value": 2.73,
            "unit": "us"
          },
          {
            "name": "10x5/predict",
            "value": 3.624,
            "unit": "us"
          },
          {
            "name": "10x5/predict_to",
            "value": 3.592,
            "unit": "us"
          },
          {
            "name": "10x5/correct",
            "value": 7.193,
            "unit": "us"
          },
          {
            "name": "10x5/correct_to",
            "value": 7.183,
            "unit": "us"
          },
          {
            "name": "15x8/predict",
            "value": 7.194,
            "unit": "us"
          },
          {
            "name": "15x8/predict_to",
            "value": 7.159,
            "unit": "us"
          },
          {
            "name": "15x8/correct",
            "value": 19.513,
            "unit": "us"
          },
          {
            "name": "15x8/correct_to",
            "value": 19.883,
            "unit": "us"
          },
          {
            "name": "20x10/predict",
            "value": 11.619,
            "unit": "us"
          },
          {
            "name": "20x10/predict_to",
            "value": 11.648,
            "unit": "us"
          },
          {
            "name": "20x10/correct",
            "value": 42.982,
            "unit": "us"
          },
          {
            "name": "20x10/correct_to",
            "value": 44.275,
            "unit": "us"
          },
          {
            "name": "nonlin/predict",
            "value": 2.5,
            "unit": "us"
          },
          {
            "name": "nonlin/predict_to",
            "value": 3.175,
            "unit": "us"
          },
          {
            "name": "nonlin/correct",
            "value": 2.743,
            "unit": "us"
          },
          {
            "name": "nonlin/correct_to",
            "value": 2.744,
            "unit": "us"
          }
        ]
      }
    ]
  }
}